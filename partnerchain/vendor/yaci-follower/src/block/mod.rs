...
pub struct BlockDataSourceImpl {
	...
	/// `security parameter / active slot coefficient` - minimal age of a block to be considered stable.
	min_slot_boundary_as_seconds: TimeDelta,
	/// `3 * security parameter / active slot coefficient` (Ouroboros Praos).
...
impl BlockDataSourceImpl {
	...
	pub async fn get_latest_stable_block_for(
		...
		let reference_timestamp = Self::timestamp_to_db_type(reference_timestamp)?;
		let latest = self.get_latest_block_info().await?;
		let offset = self.security_parameter + self.block_stability_margin;
		let stable = latest.number.0.saturating_sub(offset);
...
impl YaciBlockDataSourceConfig {
	pub fn from_env() -> std::result::Result<Self, Box<dyn Error + Send + Sync + 'static>> {
		...
		info!("Using yaci block data source configuration: {config:?}");
		Ok(config)
	}
}
...
impl BlockDataSourceImpl {
	pub async fn new_from_env(
		...
		Ok(Self::from_config(pool, YaciBlockDataSourceConfig::from_env()?, &read_mc_epoch_config()?))
	}

	pub fn from_config(
		pool: PgPool,
		YaciBlockDataSourceConfig {
	...
	async fn get_latest_block(
		...
		Ok(db_model::get_highest_block(&self.pool, max_block, min_time, max_time).await?)
	}
	...
	async fn get_stable_block_by_hash(
		...
		if let Some(block) = self.get_stable_block_by_hash_from_cache(hash.clone(), reference_timestamp)
		{
	...
	async fn get_stable_block_by_hash_from_db(
		...
		Ok(block
			...
			.filter(|(block, latest_block)| {
				block.block_no + self.security_parameter <= latest_block.block_no
	...
	async fn fill_cache(
		...
		let latest_block = db_model::get_latest_block_info(&self.pool)
			.await?
			.ok_or(InternalDataSourceError("No latest block when filling the caches.".to_string()))?;
		let latest_block_num = latest_block.block_no;
		...
		if let Ok(mut cache) = self.stable_blocks_cache.lock() {
			cache.update(blocks);
			debug!("Cached blocks {} to {} for by hash lookups.", from_block_no, to_block_no);
		}
		Ok(())
	}
