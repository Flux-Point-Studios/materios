use crate::db_model::{self, MainchainTxOutput, StakePoolEntry};
...
impl AuthoritySelectionDataSource for CandidatesDataSourceImpl {
	async fn get_ariadne_parameters(
		...
		let epoch = self.get_epoch_of_data_storage(epoch)?.0;

		let (candidates_output_opt, d_output_opt) = tokio::try_join!(
			db_model::get_token_utxo_for_epoch(&self.pool, &permissioned_candidate_policy.0, epoch),
			db_model::get_token_utxo_for_epoch(&self.pool, &d_parameter_policy.0, epoch)
		)?;
	...
	async fn get_candidates(
		...
		// stake is keyed on active_epoch == epoch (db-sync epoch_no parity).
	...
	async fn get_epoch_nonce(&self, epoch: McEpochNumber) -> Result<Option<EpochNonce>, Box<dyn std::error::Error + Send + Sync>> {
		...
		let nonce = db_model::get_epoch_nonce(&self.pool, epoch.0).await?;
		Ok(nonce.map(EpochNonce))
	}
...
impl CandidatesDataSourceImpl {
	...
	fn make_stake_map(
		...
		stake_pool_entries
			.into_iter()
			.map(|e| (MainchainKeyHash(e.pool_hash), StakeDelegation(e.stake)))
			.collect()
	}
			...
			.map(|output| {
				...
				let register_validator_datum =
					RegisterValidatorDatum::try_from(datum).map_err(|_| {
						format!("Invalid registration datum for {:?}", output.utxo_id)
					})?;
				Ok(ParsedCandidate {
					utxo_info: UtxoInfo {
						utxo_id: output.utxo_id,
						epoch_number: McEpochNumber(output.tx_epoch_no),
						block_number: McBlockNumber(output.tx_block_no),
						slot_number: McSlotNumber(output.tx_slot_no),
						tx_index_within_block: McTxIndexInBlock(output.tx_index_in_block),
					},
	...
	fn get_epoch_of_data_storage(
		...
		// Mirrors the Materios db-sync-follower preprod patch: upstream applies a
		// 2-Cardano-epoch stability offset here; on preprod (permissioned-only,
		// D=1.0) the k=432-block stable-block depth already enforced elsewhere
		// makes the additional 10-day wait dead weight. MAINNET MUST REVERT to
		// `McEpochNumber(epoch_of_data_usage.0 - 2)`.
		Ok(epoch_of_data_usage)
	}
}
