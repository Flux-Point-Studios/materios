//! Caches for immutable query results (per-epoch candidates / ariadne params),
//! mirroring `db-sync-follower`'s cache. Only stable epochs are cached.
...
impl CandidateDataSourceCached {
	...
	async fn can_use_caching_for_request(
		...
		{
			Some(stable_epoch) => {
				let stable_epoch = McEpochNumber(stable_epoch);
