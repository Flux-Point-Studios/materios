...
impl AriadneInherentDataProvider {
		...
		>,
	{
		...
		// PATCH (flux-point-studios/materios, preprod only): IOG's upstream comment
		// acknowledges that mc_reference at the LAST SLOT of data_epoch is valid —
		// they just use `<` for simpler math. The check is defense-in-depth over
		// the k=432-blocks stability depth already enforced by
		// `get_latest_stable_block_for`. Changing `<` to `<=` lets Materios use
		// Cardano data from the same epoch as its stable reference block, which
		// unblocks chain launch when the D-parameter is inserted in the current
		// Cardano epoch (otherwise the chain must wait 2 full epochs, ~10 days on
		// preprod). Preprod-only relaxation; mainnet MUST keep the stricter `<`.
