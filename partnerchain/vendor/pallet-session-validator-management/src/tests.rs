...
// [materios-patch: idp-none-fallback] Tests for the IDP-None fallback.
//
// Background: at every sc_epoch boundary (every hour on Materios preprod)
// the Ariadne IDP can return `Ok(None)` when the Cardano-side stable epoch
// hasn't caught up yet. Upstream v1.5.1 panics in that path via
//     .expect("Validator inherent data must be provided")
// causing a full chain stall until manual recovery. The patch makes the
// pallet WARN and skip the inherent for the block, reusing the current
// committee via the existing pallet-level fallback.
//
// The three tests below pin down the patched behavior. Unmodified
// upstream code panics on all three; the patched code returns / accepts
// cleanly.
mod idp_none_tests {
	use super::*;
	use crate::pallet;
	...
	/// An `InherentData` with NO data for `INHERENT_IDENTIFIER` — mirrors
	/// the production IDP-None case where the Ariadne arithmetic yields
	/// `Ok(None)` and the IDP omits the entry entirely.
	fn empty_inherent_data() -> InherentData {
		InherentData::new()
	}

	#[test]
	fn create_inherent_returns_none_when_data_missing() {
		// [materios-patch: idp-none-fallback] When the IDP has no data,
		// `create_inherent` must return `None` so the block author skips
		// the set() call. Upstream v1.5.1 panics via
		// `.expect("Validator inherent data must be provided")`.
		new_test_ext().execute_with(|| {
			let data = empty_inherent_data();
			let result =
				<SessionCommitteeManagement as ProvideInherent>::create_inherent(&data);
			assert!(
				result.is_none(),
				"create_inherent must return None when IDP data is absent, got Some(call)"
			);
		});
	}

	#[test]
	fn check_inherent_accepts_missing_inherent_when_data_absent() {
		// [materios-patch: idp-none-fallback] When a proposer skipped the
		// set() call because its IDP saw no data, verifiers with the same
		// view must accept blocks that contain no set() call. Upstream
		// panics here because `inherent_data_to_authority_selection_inputs`
		// calls `.expect(...)` on the missing data. Patched version
		// treats missing data as "proposer had same view, accept".
		new_test_ext().execute_with(|| {
			// Set genesis committee state.
			mock_pallet::CurrentEpoch::<Test>::put(189374234);
			SessionCommitteeManagement::on_initialize(1);
			let for_epoch = current_epoch_number() + 1;
			let selection_inputs_hash = SizedByteString([0u8; 32]);
			let call = pallet::Call::set {
				validators: ids_and_keys_fn(&[ALICE, BOB]),
				for_epoch_number: for_epoch,
				selection_inputs_hash,
			};
			let data = empty_inherent_data();
			// No panic expected. Accept (Ok) or deterministic error — both
			// acceptable — but NO panic must escape.
			let _ = <SessionCommitteeManagement as ProvideInherent>::check_inherent(&call, &data);
		});
	}

	#[test]
	fn is_inherent_required_returns_none_when_data_absent_even_if_next_committee_missing() {
		// [materios-patch: idp-none-fallback] `is_inherent_required` must
		// tolerate missing IDP data. Without this change, verifiers would
		// reject blocks whose authors correctly skipped the set() call,
		// causing peer-ban fork loops. See feedback_iog_idp_none_panic.md.
		new_test_ext().execute_with(|| {
			// Force NextCommittee to be absent so the upstream implementation
			// would report `Some(CommitteeNeedsToBeStoredOneEpochInAdvance)`.
			pallet::NextCommittee::<Test>::kill();
			assert!(pallet::NextCommittee::<Test>::get().is_none());

			let data = empty_inherent_data();
			let result =
				<SessionCommitteeManagement as ProvideInherent>::is_inherent_required(&data)
					.expect("is_inherent_required must not return Err when data is absent");
			assert!(
				result.is_none(),
				"is_inherent_required must return Ok(None) when IDP data is absent, got Some(err)"
			);
		});
	}
}
