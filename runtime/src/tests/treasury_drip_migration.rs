...
fn spec_version_at_least_202_and_tx_version_pinned() {
    ...
    // Spec 206 bumps transaction_version 1 → 2 because PR #26 changes the
    // `request_voucher` extrinsic wire format (adds `signatures` envelope).
    // Pre-spec-206 the wire format was stable across 202..=205. The lower
    // bound is the load-bearing invariant — never drop transaction_version.
    assert!(
        VERSION.transaction_version >= 1,
        "transaction_version must never drop below 1; got {}",
